...
- This code compiles under the Arduino IDE, but also uses the Pico SDK. This requires some awkward workarounds; particularly because the Arduino/mbed SDK 'printf' function doesn't work on Core1, and various commonplace Arduino/mbed time related functions are unstable on core1.
...
- Use an analogue servo (or digital servo if you prefer) to control steering. If your RC car donor doesn't have a servo for steering (mine has a DC motor) you should make it so. I will do that at some point. Controlling a PWM 9g servo is far easier than trying to control a DC motor, and gives you much better control. I wish I'd done that. NB the turret is best controlled by a digital servo; ideally you want accurate turret positioning at least, steering isn't so critical (until you crash).
- Use a LIDAR module, not ultrasound. They're ten times more expensive, but ultrasound is simply too slow for a fast car. You need fast accurate dependable sensing to travel at speed, and scanning ultrasound is simply not fast enough to make it reliable, so you have to cut your speed. Which sucks.
